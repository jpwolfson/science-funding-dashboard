...
def child_summary(path, name, abbrev, events, current_fy):
    stats = aggregate(events, current_fy)
...
def build(repo=REPO):
    ...
    for account in config["accounts"]:
        ...
        account_rows.append((account, events, current_fy))

    agency_children = []
    for agency_slug, events in agency_events.items():
        accounts = [(a, ev, fy) for a, ev, fy in account_rows if a["path"].split("/")[0] == agency_slug]
        ids = [e["id"] for e in events]
        if len(ids) != len(set(ids)):
            raise ValueError(f"duplicate event IDs across {agency_slug} account stores")
        current_fy = max(e["fiscalYear"] for e in events)
        children = [child_summary(f"obligations/{a['path']}", a["name"], a["abbrev"], ev, fy)
                    for a, ev, fy in accounts]
    ...
    for agency_slug, events in agency_events.items():
        account_nodes = []
        for account, _, _ in account_rows:
            ...
            account_nodes.append({"slug": account["path"].split("/")[-1],
                ...
                "path": account["path"], "children": [
                    ...
                    for pa in account["programActivities"]]})
        agency_name = next(a["agency"] for a, _, _ in account_rows
